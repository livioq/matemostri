...
- All four den phases are production art. Each `den-N.webp` is an approved
  ...
  fixed collectible slots and then Momo: background -> display ->
  collectibles -> Momo. Do not bake one layer into another.
- `pet` owns Momo's feet and height; `display` owns the cabinet box and
  its exact family slots. The empty slots are always rendered, so missing
  treasures remain legible. `denSVG` and the old `shelves` geometry remain
  as a safe missing-art fallback, but every shipped room now has separate production layers.
  `docs/DEN_ART_SPEC.json` is the approved art and layering contract, and
  tests pin every production phase's geometry to `DEN_ROOMS`.
...
`denSlotPoints`, `denLevelOf`, `denSVG`, `denBackdrop`, `denDisplay`, `denProps`,
...
rooms. The JSON is the approved separate-layer contract; `tools/den-art-spec.js`
regenerates the geometry guide and refreshes its code-owned coordinates.
